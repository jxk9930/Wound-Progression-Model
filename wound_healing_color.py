"""Color utilities for the healing-only wound progression model."""
...
def foot_mask_from_roboflow(result: dict, image_shape: tuple[int, int]) -> np.ndarray:
    h, w = image_shape
    mask = np.ones((h, w), dtype=bool)
    try:
        from PIL import Image, ImageDraw
        preds = result.get('predictions') or result.get('predicted_classes') or []
        if not preds:
            return mask
        canvas = Image.new('L', (w, h), 0)
        draw = ImageDraw.Draw(canvas)
        for pred in preds:
            pts = pred.get('points') or pred.get('polygon') or pred.get('xyn')
            if not pts:
                continue
            xy = []
            for pt in pts:
        ...
        arr = np.array(canvas, dtype=np.uint8) > 0
        return arr if arr.any() else mask
    except Exception:
        return mask
...
def compute_skin_hsv_map(rgb_image: np.ndarray, wound_mask: np.ndarray,
                         foot_mask: np.ndarray | None = None, ring_px: int = 6) -> np.ndarray:
    skin_rgb = compute_skin_rgb_map(rgb_image, wound_mask, foot_mask=foot_mask, ring_px=ring_px)
    return rgb_to_hsv(skin_rgb)
